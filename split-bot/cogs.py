...
from v2 import (
    ...
    build_check_friends, build_check_groups, build_database_section,
    build_lookup_main, build_lookup_exploit,
    build_lookup_friends, build_lookup_groups,
...
class CheckCog(commands.Cog):
    ...
    @app_commands.command(name="check",
        description="Checks whether a user is flagged in a database. (Dropdown version)")
    ...
    async def check(self, interaction, user_id: str, roblox: bool = False,
        ...
        is_admin = await _is_admin(self.bot, interaction.user)

        view = _CheckView(user, agg, extra, self.bot.storage, is_admin,
                          interaction.user.id, user_id)
        await send_v2(interaction, view.build(), view=view, ephemeral=private)
        await send_command_log(self.bot, interaction, "check",
...
class _CheckView(discord.ui.View):
    def __init__(self, user, agg, extra, storage, is_admin, invoker_id, target_id):
        super().__init__(timeout=300)
        self.user, self.agg, self.extra = user, agg, extra
        self.storage, self.is_admin     = storage, is_admin
        self.invoker_id, self.target_id = invoker_id, target_id
        self.section = "overview"; self.page = 0
    ...
    def build(self) -> dict:
        ...
        if s == "friends":   return build_check_friends(self.agg)
        if s == "groups":    return build_check_groups(self.agg)
        if s == "database":  return build_database_section(self.target_id, self.storage, self.is_admin)
...
class _CheckSelect(discord.ui.Select):
    def __init__(self, view_ref):
        options = [
            ...
            discord.SelectOption(label="Database",  value="database", description="Admin only"),
        ]
    ...
    async def callback(self, interaction):
        ...
        if interaction.user.id != vr.invoker_id:
            return await interaction.response.send_message("Not your lookup.", ephemeral=True)
        sel = self.values[0]
        if sel == "database" and not vr.is_admin:
            return await interaction.response.defer()
        vr.section = sel; vr.page = 0; vr.refresh_nav()
...
class LookupCog(commands.Cog):
    ...
    @app_commands.command(name="lookup",
        description="Checks whether a user is flagged in a database. (Card version)")
    @app_commands.describe(
        user_id="Discord user ID (roblox=False) or Roblox user ID (roblox=True).",
        roblox="True = Roblox lookup (Rotector + Moco-co). False = all Discord APIs.",
        extra="Show which API detected each server.",
    ...
    @app_commands.allowed_installs(guilds=True, users=True)
    @app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
    async def lookup(self, interaction, user_id: str, roblox: bool = False,
                     extra: bool = False, private: bool = True):
        await interaction.response.defer(ephemeral=private, thinking=True)
        disabled = self.bot.storage.get_disabled_apis()
        agg = await self.bot.detection.lookup(user_id=user_id, roblox_only=roblox, disabled_apis=disabled)
        ...
        view = _LookupView(user, agg, extra, interaction.user.id)
        cards = view.build()
        await send_v2(interaction, *cards, view=view, ephemeral=private)
        await send_command_log(self.bot, interaction, "lookup",
            {"user_id": user_id, "roblox": roblox, "extra": extra},
            agg, str(user) if user else f"User {user_id}", user_id)


class _LookupView(discord.ui.View):
    def __init__(self, user, agg, extra, invoker_id):
        super().__init__(timeout=300)
        self.user, self.agg, self.extra = user, agg, extra
        self.invoker_id = invoker_id; self.page = 0
    ...
    def build(self) -> list:
        ...
        friends = build_lookup_friends(self.user, self.agg)
        groups  = build_lookup_groups(self.user, self.agg)
        if friends: cards.append(friends)
        if groups:  cards.append(groups)
        return cards
