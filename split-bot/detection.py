...
async def _fetch_rotector(session, user_id: str) -> dict:
    return await _get(session,
        f"{config.ROTECTOR_API_BASE}/v1/lookup/roblox/user/{user_id}",
...
class DetectionService:
    ...
    async def lookup(self, user_id: str, roblox_only: bool = False, disabled_apis: set = None) -> AggregateResult:
        ...
        if roblox_only:
            # Roblox mode: only Rotector + Moco-co
            ...
            # Discord mode: all 5 APIs + RoCleaner
        ...
        # RoCleaner — local imported database (runs in both modes)
        if self._storage and "ROCLEANER" not in disabled_apis:
    ...
    def scan_database(self, user_id: str) -> AggregateResult:
        """RoCleaner-only lookup for /databasescan command."""
        agg = AggregateResult(discord_id=user_id)
        servers = self._storage.get_rocleaner_servers(user_id) if self._storage else []
        if servers:
            agg.rocleaner_servers = servers
            agg.rocleaner_flagged = True
            agg.sources_flagged.append("RoCleaner")
        agg.sources_checked = ["RoCleaner"]
        return agg
