    ...
    async def setup_hook(self) -> None:
        ...
        try:
            if config.DEV_GUILD_ID:
                ...
                log.info("Synced %d commands to guild %s", len(synced), config.DEV_GUILD_ID)
            else:
                synced = await self.tree.sync()
                log.info("Synced %d commands globally", len(synced))
        except discord.Forbidden:
            log.error("SYNC FAILED — 403. Re-invite with applications.commands scope.")
            try:
                synced = await self.tree.sync()
                log.info("Global sync fallback: %d commands", len(synced))
            except Exception as exc:
                log.error("Global sync also failed: %s", exc)
